...
- [Query Modes](#query-modes) - Direct file queries vs aliased sources
- [Request/Response Format](#requestresponse-format) - JSON input/output structure
- [File Types](#file-types) - Supported formats and auto-detection
- [Source Types](#source-types-for-aliases) - File, database, and cloud sources
- [Workflow](#workflow) - Recommended analysis steps
- [Utility Statements](#utility-statements) - DESCRIBE, SUMMARIZE, SHOW, etc.
- [Error Handling](#error-handling) - Handling errors and truncation
...
Store credentials securely in a YAML file instead of embedding them in requests. See [SECRETS.md](SECRETS.md) for complete documentation.
...
Secrets file supports: PostgreSQL, MySQL, S3, GCS, Azure, R2, HTTP, HuggingFace, Iceberg, and DuckLake.

### Security Best Practices

1. **Keep secrets.yaml in .gitignore** - Never commit credentials to version control
2. **Use environment variables** - Store actual passwords in env vars using `${VAR_NAME}` syntax
